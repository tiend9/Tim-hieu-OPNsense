...
## II. Hướng dẫn cấu hình QoS network trong OPS
...
service_plugins = router,qos # Cung cấp router Service, Bật dịch QoS
```
...
``` sh
[ml2]
extension_drivers = port_security, qos # Hỗ trọ Security Group, QoS
```
...
``` sh
[agent]
extensions = qos # thực thi QoS rule trên switch (OVS)
```
